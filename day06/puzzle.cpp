...
namespace day6 {
    ...
    pair<int, int> intersection(const vector<node>& a, const vector<node>& b) {
    ...
    int orbit_count_checksum()
    {
        ...
        for (auto [name, node] : graph.nodes)
            checksum += parent_list(node).size();
    ...
    int steps_required()
    {
        node* me = graph.get("YOU");
        node* santa = graph.get("SAN");

        vector<node> my_parents = parent_list(*me);
        vector<node> santas_parents = parent_list(*santa);

        auto [i, j] = intersection(my_parents, santas_parents);
        return i + j;
    }
