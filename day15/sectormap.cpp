...
optional<pair<int, int>> sector_map::find_oxygen_system() const
{
    for (int x = min_x; x <= max_x; x++)
        for (int y = min_y; y <= max_y; y++)
            if (get(x, y) == oxygen_system)
                return make_optional(make_pair(x, y));
...
bool sector_map_builder::contains(int x, int y) const {
    return find(nodes.begin(), nodes.end(), tile { x, y }) != nodes.end();
}

optional<tile_value> sector_map_builder::get(int x, int y) const {
    auto it = find(nodes.begin(), nodes.end(), tile { x, y });
    if (it != nodes.end())
