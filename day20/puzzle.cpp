 ...
 *              Z
 *
 * This map of the maze shows solid walls (#) and open passages (.). Every maze on Pluto has a start (the open tile next
 * to AA) and an end (the open tile next to ZZ). Mazes on Pluto also have portals; this maze has three pairs of portals:
 * BC, DE, and FG. When on an open tile next to one of these labels, a single step can take you to the other tile with
 * the same label. (You can only walk on . tiles; labels and empty space are not traversable.)
 *
 ...
 * Specifically, the labeled tiles around the inside edge actually connect to a smaller copy of the same maze, and
 * the smaller copy's inner labeled tiles connect to yet a smaller copy, and so on.
 *
 * When you enter the maze, you are at the outermost level; when at the outermost level, only the outer labels AA and ZZ
 * function (as the start and end, respectively); all other outer labeled tiles are effectively walls. At any other
 * level, AA and ZZ count as walls, but the other outer labeled tiles bring you one level outward.
 *
 * Your goal is to find a path through the maze that brings you back to ZZ at the outermost level of the maze.
 *
 * In the first example above, the shortest path is now the loop around the right side. If the starting level is 0, then
 * taking the previously-shortest path would pass through BC (to level 1), DE (to level 2), and FG (back to level 1).
 * Because this is not the outermost level, ZZ is a wall, and the only option is to go back around to BC, which would
 * only send you even deeper into the recursive maze.
 *
...
int shortest_path_through_portal_maze()
{
    ...
    fstream file { "day20/res/input.txt" };
    file >> maze;
    file.close();

    portal_graph graph { maze };
    return (int) graph.shortest_path();
}