 ...
 * value register, and J, the jump register. If the jump register is true at the end of the springscript program,
 ...
 * sensor range.
 *
 * Instead of ending your springcode program with WALK, use RUN. Doing this will enable extended sensor mode, capable of
...
void make_camera_snapshot(const output& out)
{
    fstream file { "day21/res/output.txt", fstream::out };
    for (char ascii : out.values)
        file << ascii;
    file.close();
}
...
{
    springscript script {
        "NOT A J",      /* if gap on A, jump */
        "NOT B T",
        "OR T J",       /* or if gap on B, jump */
        "NOT C T",
        "OR T J",       /* or if gap on C, jump */
        "AND D J",      /* if D is ok, jump */
        "WALK"
    };
    return hull_damage_reported(script);
}
...
int hull_damage_reported2()
{
    springscript script {
        ...
        "OR T J",   /* or if gap on C, jump */
        "AND D J",  /* if D is ok, jump */
        "AND E T",  /* if E is ok, jump */
        "OR H T",
        "AND T J",  /* or if H is ok, jump */
        "RUN"
    };
    return hull_damage_reported(script);
}